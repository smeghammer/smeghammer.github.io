...
/**
 * See https://blog.bitsrc.io/javascript-101-all-about-async-behavior-9b2a3a693f7a
 * 
 * The key to async await is the try/catch! 
 * That corresponds to the success or fail callbacks!!! ARGH!!!
 * 
 */
var filenames;
var readmes;
 ...
 * @param {string} fname
 * 
 * import a module and then set corresponding global variable to the returned data.
 * TODO: Should use an enum type structure really
 * 
 */
// MAIN REF: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/import

// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/import
// https://javascript.info/modules-dynamic-imports
// https://stackoverflow.com/questions/36107422/how-to-get-resolved-data-from-a-promise
...
async function getData(fname){
    ...
    let data = await import('./modules/' + fname + '.js');
    switch(fname){
        
        case "filenames":
            filenames = data[fname];
            break;
...
function getIWADForEntry(additional){
    ...
    try{
        if(additional && additional['iwad']&& additional['iwad'].length){
...
function getScreenshotsForEntry(additional,dir,match){
    ...
    try{
        if(additional && !isEmpty(additional['screenshots'])){
...
function setKeypressHandlers(){
    ...
    document.getElementById('term').addEventListener('keyup',function(evt){
        // check we have loaded the data:
        let term = document.getElementById('term').value;
        if(term && term.length >= 3){
            if(additional && filenames && readmes && wads){
                // console.log('data loaded');

                // /** test building alphabetised arrays: M*/
                // let filenames_a = [];
                // let filenames_b = [];
                // let filenames_c = [];
                // let filenames_d = [];
                ...
                //     console.log(filenames[a].filenames[0])
                //     try{
                //         let _match = filenames[a].filenames[0].match(/^[a-zA-Z]{1}/);
                //         if(_match){
                //             console.log(filenames[a].filenames[0]);
                //             eval("filenames_"+filenames[a].filenames[0].substring(0,1).toLowerCase()).push(filenames[a])

                //         }
                //         else{
                //             _match = filenames[a].filenames[0].match(/^![a-zA-Z]{1}/);
                //             console.log(filenames[a].filenames[0])
                //             filenames_other.push(filenames[a]);
                //         }                        
                //     }
                //     catch(ex){
                //         console.log(ex);
                ...
                _p1.appendChild(document.createTextNode('Hang on a sec - loading the big-arse data...'));
                let _p2 = document.createElement('p')
                _p2.appendChild(document.createTextNode('Dialogue will auto-close when loaded.'));

                msg.appendChild(_p1);
                msg.appendChild(_p2);
                console.log(msg)
                showDialogue(getOverlayFrame("getting data...",msg))
                loadJSImports();
            }
        }
 ...
 * the query textbox. May not want this behaviour
 */
// doSearch(term);
